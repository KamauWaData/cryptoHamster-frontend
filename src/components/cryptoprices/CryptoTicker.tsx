...
const symbolsToShow = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SOLUSDT', 'XRPUSDT',
...
const CryptoTicker: React.FC = () => {
    ...
    fetchTicker();
    const interval = setInterval(fetchTicker, 10000); // Refresh every minute
    return () => clearInterval(interval); // Cleanup on unmount
    }, []);

    return (
    <div className="bg-secondary text-foreground dark:bg-trendforge-900 dark:text-white py-2 overflow-hidden whitespace-nowrap w-full mt-6px">
      <div className="animate-marquee flex space-x-8 px-4">
        {data.map((coin) => {
          const change = parseFloat(coin.priceChangePercent);
          return (
            <div key={coin.symbol} className="flex space-x-2 items-center"
            >
